...
SELECT 
	...
	industry_branch_name 
FROM t_adela_havlova_sql_projekt_primary_final
WHERE food_name = 'Chléb konzumní kmínový' AND (`year` = 2006 OR `year` = 2018)
GROUP BY industry_branch_name ,`year`, food_name
ORDER BY ROUND (average_payroll_value / average_food_value, 0) DESC ;
...
SELECT 
	...
	industry_branch_name 
FROM t_adela_havlova_sql_projekt_primary_final
WHERE food_name = 'Mléko polotučné pasterované' AND (`year` = 2006 OR `year` = 2018) 
GROUP BY industry_branch_name, `year`, food_name
ORDER BY ROUND (average_payroll_value / average_food_value, 0) DESC ;
...
FROM t_adela_havlova_sql_projekt_primary_final
WHERE year_on_year_diff_average_food IS NOT NULL
GROUP BY food_name
